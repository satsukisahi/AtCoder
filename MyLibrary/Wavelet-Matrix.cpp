...
int main()
{
vector<uint64_t> t={5,4,5,5,2,1,5,6,1,3,5,5,0}; 
WaveletMatrix wm(t);

//access(i) t[i]の値を返す
rep(i,t.size())cout << wm.access(i) << ' ';
cout<< endl;
//intersect
//maxRange(s,t) t[s,t)中で最大値のindexを返す
rep(i,6)cout << wm.maxRange(3,4+i) << ' ';
cout<< endl;
//minRange(s,t) t[s,t)中で最小値のindexを返す
rep(i,6)cout << wm.minRange(3,4+i) << ' ';
cout<< endl;
//nextValue(s,t,l,u) t[s,t)中でl<=x<u となる最小の値を返す
rep(i,6)cout << wm.nextValue(3,4+i,2,6) << ' ';
cout<< endl;
//prevValue(s,t,l,u) t[s,t)中でl<=x<u となる最大の値を返す
rep(i,6)cout << wm.prevValue(3,4+i,2,6) << ' ';
cout<< endl;
//quantileRange(s,t,k) t[s,t)中のk番目に小さい値のindexを返す kは0-index
rep(i,t.size()-1)cout << wm.quantileRange(0,i+2,1) << ' ';
cout<< endl;
//rangeFreq(s,t,l,u) t[s,t)中でl<=x<u となるxの個数を返す
rep(i,6)cout << wm.rangeFreq(3,4+i,5,10) << ' ';
cout<< endl;
//rangeSum(s,t,l,u) t[s,t)中でl<=x<u となるxの和を返す
rep(i,6)cout << wm.rangeSum(3,4+i,5,10) << ' ';
cout<< endl;
//rank(c,i) t[0,i)のcの個数を返す iに要素の個数を入れるとエラーになる（最後の一つが数えられない）
rep(i,t.size()-1)cout << wm.rank(5,i+1) << ' ';
cout<< endl;
//rankAll(c,s,t) t[0,i)のcの個数,cより小の個数,cより大の個数をtupleで返す
auto rankall=wm.rankAll(5,0,t.size());
cout << get<0>(rankall) << ' ' << get<1>(rankall) << ' ' << get<2>(rankall) << ' '<< endl;
//rankLessThan(c,s,t) t[0,i)のcより小の個数の個数を返す
rep(i,t.size())cout << wm.rankLessThan(5,0,i+1)  << ' ';
cout<< endl;
//rankMoreThan t[0,i)のcより大の個数の個数を返す
rep(i,t.size())cout << wm.rankMoreThan(5,0,i+1)  << ' ';
cout<< endl;
//select(c,i) t中のi番目のcの出現位置を返す iと返す値は1-index
rep(i,6)cout << wm.select(5,i+1) << ' ';
cout<< endl;
//topk(s,t,k) t[s,t)中で出現回数が多い順にその頻度とともにk個返す
auto topk=wm.topk(0,t.size(),4);
for(auto r:topk)cout << r.first << '-' << r.second << ' ';
cout<< endl;
