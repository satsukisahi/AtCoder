...
rep(i,q){
  ll xs , xt , ys , yt ;
  cin >> xs >> xt >> ys >> yt ;
  auto itr = lower_bound(x.begin(), x.end(), xs);
  auto itr1 = upper_bound(x.begin(), x.end(), xt);
  cout << wm.rangeFreq(itr - x.begin(),itr1 - x.begin(),ys+(1e+9),yt+1+(1e+9)) << endl;
}
return 0;
}